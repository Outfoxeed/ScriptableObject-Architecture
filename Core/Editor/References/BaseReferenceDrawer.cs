...
namespace ScriptableObjectArchitecture.Editor.References
{
    ...
    public abstract class BaseReferenceDrawer : PropertyDrawer
    {
        ...
        {
            SerializedProperty spReferenceUsage = GetUsageSerializedProperty(property);
            if (spReferenceUsage is null)
            {
                return 0f;
            }
            
            var usageIntVal = spReferenceUsage.enumValueFlag;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var currentUsage = GetUsageSerializedProperty(property);
        ...
        private SerializedProperty GetUsageSerializedProperty(SerializedProperty property)
        {
            SerializedProperty rootProperty = property.Copy();
            foreach (object propertyObject in rootProperty)
            {
                if (propertyObject is not SerializedProperty serializedProperty)
                {
                    continue;
                }

                if (serializedProperty.name != "<ReferenceUsage>k__BackingField")
                {
                    continue;
                }
