...
namespace Plugins.ScriptableObject_Architecture.Core.Editor
{
    /// <summary>
    /// Credits to Unity-Atoms for the Asset Creation Menu
    /// https://github.com/unity-atoms/unity-atoms/blob/master/Packages/BaseAtoms/Editor/AtomsSearchableAssetCreationMenu.cs
    /// </summary>
    public static class ScriptableObjectArchitectureAssetCreationMenu
    {
        class StringTree<T>
        {
        ...
        [MenuItem(itemName: "Assets/Create/ScriptableObject Architecture &2", isValidateFunction: false, priority: -1)] // Adds to the project window's create menu
        public static void SearchMenu()
        {
            ...
            foreach (var type in TypeCache.GetTypesWithAttribute<CreateAssetMenuAttribute>()
                .Where(t => typeof(ScriptableObjectArchitectureObject).IsAssignableFrom(t)))
            {
        ...
        private class SearchTypeDropdown : AdvancedDropdown
        {
            ...
            {
                var root = new AdvancedDropdownItem("ScriptableObject Architecture");
